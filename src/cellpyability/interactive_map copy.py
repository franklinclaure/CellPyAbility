"""
Interactive 6x10 inner-plate map editor for CellPyAbility.

The editor creates a CSV that records the experimental meaning of each inner
96-well plate well (B-G, 2-11). The CSV is intentionally simple enough to edit
by hand and strict enough for future CLI analysis commands to validate.
"""

from __future__ import annotations

import argparse
import csv
from pathlib import Path
import tkinter as tk
from tkinter import filedialog, messagebox, ttk

import pandas as pd


INNER_ROWS = ["B", "C", "D", "E", "F", "G"]
INNER_COLUMNS = [str(i) for i in range(2, 12)]
PLATE_MAP_COLUMNS = [
    "well",
    "row",
    "column",
    "genotype",
    "treatment_type",
    "drug",
    "gradient_id",
    "gradient_axis",
    "concentration_index",
    "replicate_group",
    "replicate_index",
    "is_vehicle",
    "notes",
]


def inner_wells() -> list[str]:
    """Return the 60 supported inner wells in plate order."""
    return [f"{row}{column}" for row in INNER_ROWS for column in INNER_COLUMNS]


def split_well(well: str) -> tuple[str, str]:
    """Split a well like B2 into row and column labels."""
    return well[0], well[1:]


def blank_plate_map() -> pd.DataFrame:
    """Create an empty plate map DataFrame in the canonical CSV schema."""
    rows = []
    for well in inner_wells():
        row, column = split_well(well)
        rows.append(
            {
                "well": well,
                "row": row,
                "column": int(column),
                "genotype": "",
                "treatment_type": "",
                "drug": "",
                "gradient_id": "",
                "gradient_axis": "",
                "concentration_index": "",
                "replicate_group": "",
                "replicate_index": "",
                "is_vehicle": False,
                "notes": "",
            }
        )
    return pd.DataFrame(rows, columns=PLATE_MAP_COLUMNS)


def default_gda_plate_map(
    genotype_1: str = "Genotype 1",
    genotype_2: str = "Genotype 2",
    drug: str = "Drug",
) -> pd.DataFrame:
    """Create a map matching CellPyAbility's historical GDA assumptions."""
    df = blank_plate_map()
    for idx, row in df.iterrows():
        genotype = genotype_1 if row["row"] in ["B", "C", "D"] else genotype_2
        column = int(row["column"])
        df.at[idx, "genotype"] = genotype
        df.at[idx, "drug"] = "Vehicle" if column == 2 else drug
        df.at[idx, "treatment_type"] = "vehicle" if column == 2 else "drug_gradient"
        df.at[idx, "gradient_id"] = "vehicle" if column == 2 else f"{drug}_horizontal"
        df.at[idx, "gradient_axis"] = "" if column == 2 else "horizontal"
        df.at[idx, "concentration_index"] = 0 if column == 2 else column - 2
        df.at[idx, "is_vehicle"] = column == 2
        df.at[idx, "replicate_group"] = f"{genotype}_vehicle" if column == 2 else f"{genotype}_{drug}_dose_{column - 2}"
    return add_replicate_indices(df)


def add_replicate_indices(df: pd.DataFrame) -> pd.DataFrame:
    """Assign 1-based replicate indices inside each replicate group."""
    df = df.copy()
    df["replicate_index"] = ""
    grouped = df[df["replicate_group"].astype(str).str.len() > 0].groupby("replicate_group", sort=False)
    for _, group in grouped:
        for replicate_idx, row_idx in enumerate(group.index, start=1):
            df.at[row_idx, "replicate_index"] = replicate_idx
    return df


def save_plate_map(df: pd.DataFrame, output_csv: str | Path) -> Path:
    """Validate and save a plate map CSV."""
    output_path = Path(output_csv).expanduser().resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    validate_plate_map(df)
    df.to_csv(output_path, index=False, quoting=csv.QUOTE_MINIMAL)
    return output_path


def load_plate_map(input_csv: str | Path) -> pd.DataFrame:
    """Load and validate a plate map CSV."""
    df = pd.read_csv(input_csv)
    validate_plate_map(df)
    return df[PLATE_MAP_COLUMNS].copy()


def validate_plate_map(df: pd.DataFrame) -> None:
    """Raise ValueError if a DataFrame is not a valid CellPyAbility plate map."""
    missing = [column for column in PLATE_MAP_COLUMNS if column not in df.columns]
    if missing:
        raise ValueError(f"Plate map is missing required columns: {', '.join(missing)}")

    wells = df["well"].astype(str).tolist()
    expected_wells = inner_wells()
    if sorted(wells) != sorted(expected_wells):
        raise ValueError("Plate map must contain exactly the inner wells B-G and 2-11.")

    if len(wells) != len(set(wells)):
        raise ValueError("Plate map contains duplicate wells.")

    for _, row in df.iterrows():
        expected_row, expected_column = split_well(str(row["well"]))
        if row["row"] != expected_row or str(row["column"]) != expected_column:
            raise ValueError(f"Well metadata does not match well {row['well']}.")


class PlateMapApp(tk.Tk):
    """Tkinter plate-map editor."""

    colors = {
        "background": "#f5f7fb",
        "panel": "#ffffff",
        "border": "#c9d3df",
        "text": "#1f2933",
        "muted": "#5f6b7a",
        "empty": "#ffffff",
        "genotype_1": "#d9ecff",
        "genotype_2": "#ddf5dd",
        "vehicle": "#fff0c9",
        "gradient": "#eadcff",
        "active": "#ffd166",
        "active_border": "#2f80ed",
    }

    def __init__(self, output_csv: str | Path | None = None):
        super().__init__()
        self.title("CellPyAbility Plate Map")
        self.geometry("1360x760")
        self.minsize(1120, 680)
        self.configure(bg=self.colors["background"])

        self.output_csv = Path(output_csv).expanduser() if output_csv else None
        self.df = blank_plate_map()
        self.selected_wells: set[str] = set()
        self.dragging = False
        self.drag_seen: set[str] = set()
        self.well_bounds: dict[str, tuple[int, int, int, int]] = {}

        self.mode = tk.StringVar(value="genotype_1")
        self.genotype_1_name = tk.StringVar(value="Genotype 1")
        self.genotype_2_name = tk.StringVar(value="Genotype 2")
        self.drug_name = tk.StringVar(value="Drug")
        self.gradient_id = tk.StringVar(value="Drug gradient 1")
        self.gradient_axis = tk.StringVar(value="horizontal")
        self.reverse_gradient = tk.BooleanVar(value=False)
        self.notes = tk.StringVar(value="")

        self._build_layout()
        self.after(100, self._render_plate)
        self.after(500, self._render_plate)

    def _build_layout(self) -> None:
        sidebar = tk.Frame(self, bg=self.colors["panel"], width=340, highlightbackground=self.colors["border"], highlightthickness=1)
        sidebar.place(x=18, y=18, width=340, relheight=1.0, height=-36)

        plate_frame = tk.Frame(self, bg=self.colors["background"], width=900, height=650)
        plate_frame.place(x=384, y=18, relwidth=1.0, width=-402, relheight=1.0, height=-36)

        tk.Label(
            plate_frame,
            text="Inner 60 Wells",
            bg=self.colors["background"],
            fg=self.colors["text"],
            font=("TkDefaultFont", 18, "bold"),
        ).place(x=0, y=0, height=34)

        self.plate_grid = tk.Canvas(
            plate_frame,
            bg="#ffffff",
            highlightthickness=1,
            highlightbackground=self.colors["border"],
            width=980,
            height=560,
        )
        self.plate_grid.place(x=0, y=48, relwidth=1.0, relheight=1.0, height=-48)
        self.plate_grid.bind("<ButtonPress-1>", self.start_drag)
        self.plate_grid.bind("<B1-Motion>", self.drag_enter)
        self.plate_grid.bind("<ButtonRelease-1>", self.stop_drag)
        self.plate_grid.bind("<Configure>", lambda event: self._render_plate())

        self._build_sidebar(sidebar)

    def _build_sidebar(self, sidebar: tk.Frame) -> None:
        content = tk.Frame(sidebar, bg=self.colors["panel"])
        content.pack(fill=tk.BOTH, expand=True, padx=16, pady=16)

        self._label(content, "Plate Map Tools", font=("TkDefaultFont", 17, "bold")).pack(anchor="w")
        self._label(
            content,
            "Click or drag wells to highlight them, then apply an assignment below.",
            color=self.colors["muted"],
            wraplength=260,
        ).pack(anchor="w", pady=(4, 14))

        names = self._section(content, "Names")
        self._entry(names, "Genotype 1", self.genotype_1_name)
        self._entry(names, "Genotype 2", self.genotype_2_name)
        self._entry(names, "Drug", self.drug_name)
        self._entry(names, "Gradient ID", self.gradient_id)

        gradient = self._section(content, "Gradient Direction")
        tk.Radiobutton(
            gradient,
            text="Horizontal increase; vertical stacks are replicates",
            value="horizontal",
            variable=self.gradient_axis,
            bg=self.colors["panel"],
            fg=self.colors["text"],
            activebackground=self.colors["panel"],
            selectcolor=self.colors["panel"],
            anchor="w",
            wraplength=260,
            justify=tk.LEFT,
        ).pack(fill=tk.X, pady=1)
        tk.Radiobutton(
            gradient,
            text="Vertical increase; side-by-side wells are replicates",
            value="vertical",
            variable=self.gradient_axis,
            bg=self.colors["panel"],
            fg=self.colors["text"],
            activebackground=self.colors["panel"],
            selectcolor=self.colors["panel"],
            anchor="w",
            wraplength=260,
            justify=tk.LEFT,
        ).pack(fill=tk.X, pady=1)
        tk.Checkbutton(
            gradient,
            text="Reverse increase",
            variable=self.reverse_gradient,
            bg=self.colors["panel"],
            fg=self.colors["text"],
            activebackground=self.colors["panel"],
            selectcolor=self.colors["panel"],
            anchor="w",
        ).pack(fill=tk.X, pady=(5, 0))

        notes_box = self._section(content, "Notes")
        tk.Entry(notes_box, textvariable=self.notes, bg="#ffffff", fg=self.colors["text"], relief=tk.SOLID, bd=1).pack(fill=tk.X)

        apply_box = self._section(content, "Apply To Highlighted Wells")
        self._action_button(apply_box, "Set Genotype 1", lambda: self.apply_selection("genotype_1")).pack(fill=tk.X, pady=(0, 5))
        self._action_button(apply_box, "Set Genotype 2", lambda: self.apply_selection("genotype_2")).pack(fill=tk.X, pady=(0, 5))
        self._action_button(apply_box, "Set Vehicle Controls", lambda: self.apply_selection("vehicle")).pack(fill=tk.X, pady=(0, 5))
        self._action_button(apply_box, "Set Drug Gradient", lambda: self.apply_selection("drug_gradient")).pack(fill=tk.X, pady=(0, 5))
        self._action_button(apply_box, "Clear Highlighted", lambda: self.apply_selection("clear")).pack(fill=tk.X)

        actions = tk.Frame(content, bg=self.colors["panel"])
        actions.pack(fill=tk.X, pady=(16, 0))
        self._action_button(actions, "Clear Highlight", self.clear_selection).pack(fill=tk.X, pady=(0, 6))
        self._action_button(actions, "Reset All Wells", self.reset_map).pack(fill=tk.X, pady=(0, 6))
        self._action_button(actions, "Save Layout", self.save_dialog, primary=True).pack(fill=tk.X, pady=(6, 0))

        self.status = tk.Label(
            content,
            text="Ready. Highlight wells to begin.",
            bg=self.colors["panel"],
            fg=self.colors["muted"],
            wraplength=260,
            justify=tk.LEFT,
        )
        self.status.pack(anchor="w", fill=tk.X, pady=(14, 0))

    def _label(self, parent: tk.Widget, text: str, font=None, color: str | None = None, wraplength: int | None = None) -> tk.Label:
        return tk.Label(
            parent,
            text=text,
            bg=self.colors["panel"],
            fg=color or self.colors["text"],
            font=font,
            wraplength=wraplength or 0,
            justify=tk.LEFT,
        )

    def _section(self, parent: tk.Frame, title: str) -> tk.Frame:
        frame = tk.Frame(parent, bg=self.colors["panel"])
        frame.pack(fill=tk.X, pady=(12, 0))
        self._label(frame, title, font=("TkDefaultFont", 12, "bold")).pack(anchor="w", pady=(0, 5))
        return frame

    def _entry(self, parent: tk.Frame, label: str, variable: tk.StringVar) -> None:
        row = tk.Frame(parent, bg=self.colors["panel"])
        row.pack(fill=tk.X, pady=2)
        tk.Label(row, text=label, bg=self.colors["panel"], fg=self.colors["text"], width=10, anchor="w").pack(side=tk.LEFT)
        tk.Entry(row, textvariable=variable, bg="#ffffff", fg=self.colors["text"], relief=tk.SOLID, bd=1).pack(side=tk.LEFT, fill=tk.X, expand=True)

    def _action_button(self, parent: tk.Frame, text: str, command, primary: bool = False) -> tk.Button:
        bg = "#2f80ed" if primary else "#eef3f8"
        fg = "#ffffff" if primary else self.colors["text"]
        return tk.Button(
            parent,
            text=text,
            command=command,
            bg=bg,
            fg=fg,
            activebackground=bg,
            activeforeground=fg,
            relief=tk.FLAT,
            padx=10,
            pady=8,
        )

    def _render_plate(self) -> None:
        canvas = self.plate_grid
        canvas.delete("all")
        self.well_bounds.clear()

        width = max(canvas.winfo_width(), int(canvas.cget("width")), 760)
        height = max(canvas.winfo_height(), int(canvas.cget("height")), 480)
        canvas.create_text(
            10,
            10,
            anchor="nw",
            text=f"Plate grid ({width} x {height})",
            fill=self.colors["muted"],
            font=("TkDefaultFont", 10),
        )
        left = 48
        top = 42
        gap = 10
        label_space = 28
        usable_width = width - left - 24
        usable_height = height - top - 24
        if usable_width < 680 or usable_height < 390:
            canvas.create_text(
                20,
                36,
                anchor="nw",
                text="Plate grid needs more window space. Widen this window.",
                fill=self.colors["text"],
                font=("TkDefaultFont", 14, "bold"),
            )
            return
        cell_width = max(64, int((usable_width - (len(INNER_COLUMNS) - 1) * gap) / len(INNER_COLUMNS)))
        cell_height = max(58, int((usable_height - label_space - (len(INNER_ROWS) - 1) * gap) / len(INNER_ROWS)))

        for col_idx, column in enumerate(INNER_COLUMNS):
            x = left + col_idx * (cell_width + gap)
            canvas.create_text(
                x + cell_width / 2,
                16,
                text=column,
                fill=self.colors["muted"],
                font=("TkDefaultFont", 12, "bold"),
            )

        for row_idx, row in enumerate(INNER_ROWS):
            y = top + label_space + row_idx * (cell_height + gap)
            canvas.create_text(
                20,
                y + cell_height / 2,
                text=row,
                fill=self.colors["muted"],
                font=("TkDefaultFont", 12, "bold"),
            )
            for col_idx, column in enumerate(INNER_COLUMNS):
                well = f"{row}{column}"
                x = left + col_idx * (cell_width + gap)
                x2 = x + cell_width
                y2 = y + cell_height
                fill = self._well_color(well)
                outline = self.colors["active_border"] if well in self.selected_wells else self.colors["border"]
                stroke = 3 if well in self.selected_wells else 1
                self.well_bounds[well] = (x, y, x2, y2)
                canvas.create_rectangle(x, y, x2, y2, fill=fill, outline=outline, width=stroke)
                canvas.create_text(
                    x + cell_width / 2,
                    y + cell_height / 2,
                    text=self._button_text(well),
                    fill=self.colors["text"],
                    font=("TkDefaultFont", 11),
                    justify=tk.CENTER,
                    width=cell_width - 12,
                )

    def _button_text(self, well: str) -> str:
        row = self.df[self.df["well"] == well].iloc[0]
        lines = [well]
        if row["genotype"]:
            lines.append(str(row["genotype"])[:16])
        if row["treatment_type"] == "vehicle":
            lines.append("Vehicle")
        elif row["treatment_type"] == "drug_gradient":
            lines.append(f"{row['drug']} #{row['concentration_index']}")
        return "\n".join(lines)

    def _well_color(self, well: str) -> str:
        if well in self.selected_wells:
            return self.colors["active"]
        row = self.df[self.df["well"] == well].iloc[0]
        if row["treatment_type"] == "vehicle":
            return self.colors["vehicle"]
        if row["treatment_type"] == "drug_gradient":
            return self.colors["gradient"]
        if row["genotype"] == self.genotype_1_name.get():
            return self.colors["genotype_1"]
        if row["genotype"] == self.genotype_2_name.get():
            return self.colors["genotype_2"]
        return self.colors["empty"]

    def refresh_buttons(self) -> None:
        self._render_plate()

    def well_at_event(self, event: tk.Event) -> str | None:
        for well, (x1, y1, x2, y2) in self.well_bounds.items():
            if x1 <= event.x <= x2 and y1 <= event.y <= y2:
                return well
        return None

    def start_drag(self, event: tk.Event) -> None:
        well = self.well_at_event(event)
        if well is None:
            return
        self.dragging = True
        self.drag_seen = set()
        self.toggle_selection(well)

    def drag_enter(self, event: tk.Event) -> None:
        if not self.dragging:
            return
        well = self.well_at_event(event)
        if well is None or well in self.drag_seen:
            return
        self.drag_seen.add(well)
        self.selected_wells.add(well)
        self.status.configure(text=f"{len(self.selected_wells)} well(s) highlighted.")
        self.refresh_buttons()

    def stop_drag(self, event: tk.Event | None = None) -> None:
        self.dragging = False
        self.drag_seen = set()

    def toggle_selection(self, well: str) -> None:
        if well in self.drag_seen:
            return
        self.drag_seen.add(well)
        if well in self.selected_wells:
            self.selected_wells.remove(well)
        else:
            self.selected_wells.add(well)
        self.status.configure(text=f"{len(self.selected_wells)} well(s) highlighted.")
        self.refresh_buttons()

    def clear_selection(self) -> None:
        self.selected_wells.clear()
        self.status.configure(text="Highlight cleared.")
        self.refresh_buttons()

    def apply_selection(self, mode: str) -> None:
        if not self.selected_wells:
            messagebox.showinfo("No wells highlighted", "Highlight one or more wells first.")
            return

        wells = self._ordered_wells(list(self.selected_wells), self.gradient_axis.get())
        if mode == "genotype_1":
            self._set_wells(wells, genotype=self.genotype_1_name.get())
        elif mode == "genotype_2":
            self._set_wells(wells, genotype=self.genotype_2_name.get())
        elif mode == "vehicle":
            self._set_vehicle(wells)
        elif mode == "drug_gradient":
            self._set_gradient(wells)
        elif mode == "clear":
            self._clear_wells(wells)

        self.df = add_replicate_indices(self.df)
        count = len(self.selected_wells)
        self.selected_wells.clear()
        self.status.configure(text=f"Applied {mode.replace('_', ' ')} to {count} well(s).")
        self.refresh_buttons()

    def _set_wells(self, wells: list[str], **values: object) -> None:
        for well in wells:
            idx = self.df.index[self.df["well"] == well][0]
            for key, value in values.items():
                self.df.at[idx, key] = value
            if self.notes.get():
                self.df.at[idx, "notes"] = self.notes.get()

    def _set_vehicle(self, wells: list[str]) -> None:
        for well in wells:
            idx = self.df.index[self.df["well"] == well][0]
            genotype = self.df.at[idx, "genotype"] or ""
            self.df.at[idx, "treatment_type"] = "vehicle"
            self.df.at[idx, "drug"] = "Vehicle"
            self.df.at[idx, "gradient_id"] = "vehicle"
            self.df.at[idx, "gradient_axis"] = ""
            self.df.at[idx, "concentration_index"] = 0
            self.df.at[idx, "replicate_group"] = f"{genotype}_vehicle" if genotype else "vehicle"
            self.df.at[idx, "is_vehicle"] = True
            if self.notes.get():
                self.df.at[idx, "notes"] = self.notes.get()

    def _set_gradient(self, wells: list[str]) -> None:
        axis = self.gradient_axis.get()
        drug = self.drug_name.get().strip() or "Drug"
        gradient_id = self.gradient_id.get().strip() or f"{drug}_{axis}"
        ordered = self._ordered_wells(wells, axis)
        concentration_by_well = self._concentration_indices(ordered, axis)

        for well in ordered:
            idx = self.df.index[self.df["well"] == well][0]
            genotype = self.df.at[idx, "genotype"] or ""
            concentration_index = concentration_by_well[well]
            row, column = split_well(well)
            replicate_key = column if axis == "horizontal" else row
            self.df.at[idx, "treatment_type"] = "drug_gradient"
            self.df.at[idx, "drug"] = drug
            self.df.at[idx, "gradient_id"] = gradient_id
            self.df.at[idx, "gradient_axis"] = axis
            self.df.at[idx, "concentration_index"] = concentration_index
            self.df.at[idx, "replicate_group"] = f"{genotype}_{gradient_id}_dose_{concentration_index}_{replicate_key}".strip("_")
            self.df.at[idx, "is_vehicle"] = False
            if self.notes.get():
                self.df.at[idx, "notes"] = self.notes.get()

    def _ordered_wells(self, wells: list[str], axis: str) -> list[str]:
        wells = sorted(wells, key=lambda well: (INNER_ROWS.index(well[0]), int(well[1:])))
        if axis == "horizontal":
            wells = sorted(wells, key=lambda well: (int(well[1:]), INNER_ROWS.index(well[0])))
        if self.reverse_gradient.get():
            wells = list(reversed(wells))
        return wells

    def _concentration_indices(self, wells: list[str], axis: str) -> dict[str, int]:
        if axis == "horizontal":
            ordered_columns = sorted({int(well[1:]) for well in wells}, reverse=self.reverse_gradient.get())
            lookup = {str(column): idx for idx, column in enumerate(ordered_columns, start=1)}
            return {well: lookup[well[1:]] for well in wells}

        ordered_rows = sorted({well[0] for well in wells}, key=INNER_ROWS.index, reverse=self.reverse_gradient.get())
        lookup = {row: idx for idx, row in enumerate(ordered_rows, start=1)}
        return {well: lookup[well[0]] for well in wells}

    def _clear_wells(self, wells: list[str]) -> None:
        for well in wells:
            idx = self.df.index[self.df["well"] == well][0]
            for column in PLATE_MAP_COLUMNS:
                if column not in {"well", "row", "column"}:
                    self.df.at[idx, column] = False if column == "is_vehicle" else ""

    def load_default(self) -> None:
        self.df = default_gda_plate_map(
            genotype_1=self.genotype_1_name.get(),
            genotype_2=self.genotype_2_name.get(),
            drug=self.drug_name.get(),
        )
        self.status.configure(text="Loaded the current default GDA-style map.")
        self.refresh_buttons()

    def reset_map(self) -> None:
        if messagebox.askyesno("Reset all wells", "Clear every well and return to the original blank layout?"):
            self.df = blank_plate_map()
            self.selected_wells.clear()
            self.status.configure(text="All wells reset to the original blank layout.")
            self.refresh_buttons()

    def save_dialog(self) -> None:
        initialfile = self.output_csv.name if self.output_csv else "plate_map.csv"
        selected = filedialog.asksaveasfilename(
            title="Save plate map layout",
            defaultextension=".csv",
            initialfile=initialfile,
            filetypes=[("CSV files", "*.csv"), ("All files", "*.*")],
        )
        if not selected:
            return
        output = Path(selected)

        try:
            saved = save_plate_map(add_replicate_indices(self.df), output)
        except ValueError as exc:
            messagebox.showerror("Invalid plate map", str(exc))
            return

        self.output_csv = saved
        messagebox.showinfo("Plate map saved", f"Saved plate map to:\n{saved}")
        self.status.configure(text=f"Saved {saved}")


def launch_plate_map_matplotlib(output_csv: str | Path | None = None) -> None:
    """Launch the default Matplotlib plate-map editor."""
    import matplotlib.pyplot as plt
    from matplotlib.patches import Rectangle
    from matplotlib.widgets import Button, CheckButtons, RadioButtons, TextBox

    colors = {
        "background": "#f5f7fb",
        "panel": "#ffffff",
        "border": "#c9d3df",
        "text": "#1f2933",
        "muted": "#5f6b7a",
        "unassigned": "#f8fafc",
        "genotype_1": "#6495ed",
        "genotype_2": "#ff2400",
        "vehicle_g1": "#d8e6ff",
        "vehicle_g2": "#ffd6cc",
        "vehicle_unknown": "#fef3c7",
        "drug_g1": "#b7caf7",
        "drug_g2": "#ffb3a3",
        "drug_unknown": "#f5d0fe",
        "gradient_g1_light": "#d8e6ff",
        "gradient_g1_dark": "#8fb7ff",
        "gradient_g2_light": "#ffd6cc",
        "gradient_g2_dark": "#ff8b78",
        "gradient_unknown_light": "#e9d5ff",
        "gradient_unknown_dark": "#c084fc",
        "active": "#ffd166",
        "active_border": "#2f80ed",
        "pattern": "#314155",
    }
    rows = [chr(ord("A") + idx) for idx in range(8)]
    columns = [str(idx) for idx in range(1, 13)]
    output_base = Path(output_csv).expanduser() if output_csv else Path.cwd() / "plate_map.csv"
    output_dir = output_base.parent if output_base.suffix.lower() == ".csv" else output_base
    initial_layout_name = output_base.stem if output_base.suffix.lower() == ".csv" else "plate_map"

    def blank_map() -> pd.DataFrame:
        records = []
        for row in rows:
            for column in columns:
                records.append(
                    {
                        "well": f"{row}{column}",
                        "row": row,
                        "column": int(column),
                        "genotype": "",
                        "treatment_type": "",
                        "drug": "",
                        "gradient_id": "",
                        "gradient_axis": "",
                        "concentration_index": "",
                        "replicate_group": "",
                        "replicate_index": "",
                        "is_vehicle": False,
                        "notes": "",
                    }
                )
        return pd.DataFrame(records, columns=PLATE_MAP_COLUMNS)

    data = {"df": blank_map()}
    selected_wells: set[str] = set()
    state = {
        "dragging": False,
        "drag_start": None,
        "drag_end": None,
        "marquee": None,
        "confirm_reset": False,
        "drug_count": 3,
        "selected_drug": "Drug1",
        "gradient_axis": "horizontal",
        "reverse": False,
    }
    controls: dict[str, object] = {}

    fig = plt.figure(figsize=(14, 8), facecolor=colors["background"])
    fig.canvas.manager.set_window_title("CellPyAbility Plate Map")
    plate_ax = fig.add_axes([0.34, 0.17, 0.62, 0.72], facecolor=colors["panel"])
    detail_ax = fig.add_axes([0.34, 0.055, 0.62, 0.075], facecolor=colors["panel"])
    detail_ax.set_xticks([])
    detail_ax.set_yticks([])
    for spine in detail_ax.spines.values():
        spine.set_edgecolor(colors["border"])
    fig.text(0.34, 0.935, "96-Well Plate", fontsize=18, fontweight="bold", color=colors["text"])
    status_text = fig.text(0.105, 0.035, "Ready. Drag a rectangle over wells to highlight.", fontsize=10, color=colors["muted"])
    detail_text = detail_ax.text(
        0.02,
        0.5,
        "Hover over a well for details.",
        ha="left",
        va="center",
        color=colors["text"],
        fontsize=11,
        transform=detail_ax.transAxes,
    )

    def set_status(message: str) -> None:
        status_text.set_text(message)
        fig.canvas.draw_idle()

    def set_detail(message: str) -> None:
        detail_text.set_text(message)
        fig.canvas.draw_idle()

    def make_textbox(y: float, label: str, initial: str) -> TextBox:
        ax = fig.add_axes([0.105, y, 0.19, 0.035], facecolor=colors["panel"])
        ax.format_coord = lambda x, y: ""
        return TextBox(ax, label, initial=initial, color=colors["panel"], hovercolor="#eef3f8")

    controls["layout_name"] = make_textbox(0.885, "Layout name: ", initial_layout_name)
    set_g1_ax = fig.add_axes([0.105, 0.82, 0.19, 0.04])
    set_g2_ax = fig.add_axes([0.105, 0.765, 0.19, 0.04])
    vehicle_ax = fig.add_axes([0.105, 0.71, 0.19, 0.04])
    controls["drug_count"] = make_textbox(0.66, "Number of drugs: ", "3")
    fig.text(0.105, 0.615, "Drug Gradient Direction:", fontsize=11, fontweight="bold", color=colors["text"])
    direction_ax = fig.add_axes([0.105, 0.555, 0.12, 0.048], facecolor=colors["panel"])
    direction_ax.format_coord = lambda x, y: ""
    direction_radio = RadioButtons(direction_ax, ("Horizontal", "Vertical"), active=0, activecolor=colors["active_border"])
    reverse_ax = fig.add_axes([0.23, 0.561, 0.065, 0.035], facecolor=colors["panel"])
    reverse_ax.format_coord = lambda x, y: ""
    reverse_check = CheckButtons(reverse_ax, ("Reverse",), (False,))
    gradient_ax = fig.add_axes([0.105, 0.245, 0.19, 0.04])
    clear_ax = fig.add_axes([0.105, 0.19, 0.19, 0.04])
    reset_ax = fig.add_axes([0.105, 0.125, 0.19, 0.04])
    save_ax = fig.add_axes([0.105, 0.07, 0.19, 0.04])

    def textbox_value(key: str) -> str:
        return controls[key].text.strip()

    def row_df_index(well: str) -> int:
        df = data["df"]
        return int(df.index[df["well"] == well][0])

    def get_row(well: str) -> pd.Series:
        return data["df"][data["df"]["well"] == well].iloc[0]

    def enabled_well(well: str) -> bool:
        row, column = split_well(well)
        return row in rows and column in columns

    def drug_labels() -> list[str]:
        try:
            count = int(float(textbox_value("drug_count") or state["drug_count"]))
        except ValueError:
            count = int(state["drug_count"])
        count = max(1, min(12, count))
        state["drug_count"] = count
        labels = [f"Drug{i}" for i in range(1, count + 1)]
        if state["selected_drug"] not in labels:
            state["selected_drug"] = labels[0]
        return labels

    def drug_pattern(value: object) -> str:
        text = str(value or "").strip()
        return {
            "Drug1": "..",
            "Drug2": "///",
            "Drug3": "|||",
            "Drug4": "---",
            "Drug5": "",
        }.get(text, "")

    def select_drug(label: str) -> None:
        state["selected_drug"] = label
        direction = str(state["gradient_axis"]).capitalize()
        reverse = "reverse" if state["reverse"] else "forward"
        set_status(f"{label} selected: {direction}, {reverse}.")

    def refresh_drug_toggles(text: str | None = None) -> None:
        for axis in controls.get("drug_axes", []):
            axis.remove()
        labels = drug_labels()
        display_labels = [label.replace("Drug", "Drug ") for label in labels]
        height = min(0.25, 0.026 * len(labels) + 0.018)
        drug_ax = fig.add_axes([0.105, 0.305, 0.19, height], facecolor=colors["panel"])
        drug_ax.format_coord = lambda x, y: ""
        drug_radio = RadioButtons(drug_ax, display_labels, active=labels.index(state["selected_drug"]), activecolor=colors["active_border"])
        drug_axes = [drug_ax]
        row_step = height / max(1, len(labels))
        icon_size = min(0.024, row_step * 0.75)
        for idx, label in enumerate(labels):
            icon_y = 0.305 + height - row_step * (idx + 0.72)
            icon_ax = fig.add_axes([0.27, icon_y, icon_size, icon_size], facecolor=colors["panel"])
            icon_ax.format_coord = lambda x, y: ""
            icon_ax.set_xticks([])
            icon_ax.set_yticks([])
            for spine in icon_ax.spines.values():
                spine.set_visible(False)
            icon_ax.add_patch(
                Rectangle(
                    (0.08, 0.08),
                    0.84,
                    0.84,
                    facecolor="#ffffff",
                    edgecolor=colors["pattern"],
                    linewidth=0.8,
                    hatch=drug_pattern(label),
                )
            )
            icon_ax.set_xlim(0, 1)
            icon_ax.set_ylim(0, 1)
            drug_axes.append(icon_ax)

        def choose_drug(choice: str) -> None:
            select_drug(choice.replace(" ", ""))

        drug_radio.on_clicked(choose_drug)
        controls["drug_axes"] = drug_axes
        controls["drug_radio"] = drug_radio
        set_status(f"{len(labels)} drug toggle(s) available.")
        fig.canvas.draw_idle()

    controls["drug_count"].on_submit(refresh_drug_toggles)
    if hasattr(controls["drug_count"], "on_text_change"):
        controls["drug_count"].on_text_change(refresh_drug_toggles)

    def set_direction(choice: str) -> None:
        state["gradient_axis"] = "vertical" if choice.startswith("Vertical") else "horizontal"
        select_drug(str(state["selected_drug"]))

    def set_reverse(_choice: str) -> None:
        state["reverse"] = not state["reverse"]
        select_drug(str(state["selected_drug"]))

    direction_radio.on_clicked(set_direction)
    reverse_check.on_clicked(set_reverse)

    def blend_hex(start: str, end: str, amount: float) -> str:
        amount = max(0.0, min(1.0, amount))
        start_rgb = tuple(int(start[i : i + 2], 16) for i in (1, 3, 5))
        end_rgb = tuple(int(end[i : i + 2], 16) for i in (1, 3, 5))
        rgb = tuple(round(a + (b - a) * amount) for a, b in zip(start_rgb, end_rgb))
        return "#{:02x}{:02x}{:02x}".format(*rgb)

    def genotype_code(value: object) -> str:
        if value == "Genotype 1":
            return "g1"
        if value == "Genotype 2":
            return "g2"
        return "UN"

    def drug_code(value: object) -> str:
        text = str(value or "").strip()
        if not text or text == "Vehicle":
            return "UN"
        if text.lower().startswith("drug"):
            suffix = text[4:].strip()
            if suffix.isdigit():
                return f"d{suffix}"
        digits = "".join(character for character in text if character.isdigit())
        return f"d{digits}" if digits else "UN"

    def concentration_code(value: object) -> str:
        text = str(value or "").strip()
        return f"c{text}" if text.isdigit() else "UN"

    def well_codes(well: str) -> dict[str, str]:
        row = get_row(well)
        g_code = genotype_code(row["genotype"])
        d_code = drug_code(row["drug"])
        c_code = concentration_code(row["concentration_index"]) if row["treatment_type"] == "drug_gradient" else "UN"
        v_code = "v" if bool(row["is_vehicle"]) or row["treatment_type"] == "vehicle" else "UN"
        if g_code == "UN" and d_code == "UN" and c_code == "UN" and v_code == "UN":
            compact = "0"
        elif g_code != "UN" and v_code == "v":
            compact = f"{g_code}v"
        elif g_code != "UN" and d_code != "UN" and c_code != "UN":
            compact = f"{g_code}{d_code}{c_code}"
        else:
            compact = "UN"
        return {"g": g_code, "d": d_code, "c": c_code, "v": v_code, "compact": compact}

    def compact_csv_value(well: str) -> str:
        codes = well_codes(well)
        return codes["compact"] if codes["compact"] != "UN" else "0"

    def hover_detail(well: str | None) -> str:
        if well is None:
            return "Hover over a well for details."
        codes = well_codes(well)
        return f"{well} | g: {codes['g']} | d: {codes['d']} | c: {codes['c']} | v: {codes['v']}"

    def well_text(well: str) -> str:
        codes = well_codes(well)
        if codes["compact"] not in {"0", "UN"}:
            return f"{well}\n{codes['compact']}"
        return well

    def well_pattern(well: str) -> str:
        row = get_row(well)
        if row["drug"] == "Vehicle":
            return ""
        return drug_pattern(row["drug"])

    def well_color(well: str) -> str:
        if well in selected_wells:
            return colors["active"]
        row = get_row(well)
        genotype = row["genotype"]
        if row["treatment_type"] == "vehicle":
            if genotype == "Genotype 1":
                return colors["vehicle_g1"]
            if genotype == "Genotype 2":
                return colors["vehicle_g2"]
            return colors["vehicle_unknown"]
        if row["treatment_type"] == "drug_gradient":
            try:
                amount = ((int(row["concentration_index"]) - 1) / 8) * 0.62
            except (TypeError, ValueError):
                amount = 0.0
            if genotype == "Genotype 1":
                return blend_hex(colors["gradient_g1_light"], colors["gradient_g1_dark"], amount)
            if genotype == "Genotype 2":
                return blend_hex(colors["gradient_g2_light"], colors["gradient_g2_dark"], amount)
            return blend_hex(colors["gradient_unknown_light"], colors["gradient_unknown_dark"], amount)
        if row["drug"]:
            if genotype == "Genotype 1":
                return colors["drug_g1"]
            if genotype == "Genotype 2":
                return colors["drug_g2"]
            return colors["drug_unknown"]
        if genotype == "Genotype 1":
            return colors["genotype_1"]
        if genotype == "Genotype 2":
            return colors["genotype_2"]
        return colors["unassigned"]

    def row_index(row: str) -> int:
        return rows.index(row)

    def ordered_wells(wells: list[str], axis: str, reverse: bool = False) -> list[str]:
        ordered = sorted(wells, key=lambda well: (row_index(well[0]), int(well[1:])))
        if axis == "horizontal":
            ordered = sorted(ordered, key=lambda well: (int(well[1:]), row_index(well[0])))
        if reverse:
            ordered = list(reversed(ordered))
        return ordered

    def concentration_indices(wells: list[str], axis: str, reverse: bool = False) -> dict[str, int]:
        if axis == "horizontal":
            ordered_columns = sorted({int(well[1:]) for well in wells}, reverse=reverse)
            lookup = {str(column): idx for idx, column in enumerate(ordered_columns, start=1)}
            return {well: lookup[well[1:]] for well in wells}
        ordered_rows = sorted({well[0] for well in wells}, key=row_index, reverse=reverse)
        lookup = {row: idx for idx, row in enumerate(ordered_rows, start=1)}
        return {well: lookup[well[0]] for well in wells}

    def well_from_event(event) -> str | None:
        if event.inaxes is not plate_ax or event.xdata is None or event.ydata is None:
            return None
        col_idx = int(event.xdata)
        row_idx = int(event.ydata)
        if 0 <= col_idx < len(columns) and 0 <= row_idx < len(rows):
            return f"{rows[row_idx]}{columns[col_idx]}"
        return None

    def wells_in_rectangle(start: tuple[float, float], end: tuple[float, float]) -> set[str]:
        x1, y1 = start
        x2, y2 = end
        xmin, xmax = sorted([x1, x2])
        ymin, ymax = sorted([y1, y2])
        hits = set()
        for row_idx, row in enumerate(rows):
            for col_idx, column in enumerate(columns):
                cell_x1, cell_x2 = col_idx, col_idx + 0.9
                cell_y1, cell_y2 = row_idx, row_idx + 0.9
                if xmax >= cell_x1 and xmin <= cell_x2 and ymax >= cell_y1 and ymin <= cell_y2:
                    hits.add(f"{row}{column}")
        return hits

    def redraw_plate() -> None:
        plate_ax.clear()
        plate_ax.format_coord = lambda x, y: ""
        plate_ax.set_xlim(-0.7, len(columns))
        plate_ax.set_ylim(len(rows), -0.7)
        plate_ax.set_aspect("auto")
        plate_ax.axis("off")
        bbox = plate_ax.get_window_extent()
        fontsize = max(5, min(10, bbox.width / 132))
        label_fontsize = max(7, min(12, bbox.width / 120))
        for col_idx, column in enumerate(columns):
            plate_ax.text(col_idx + 0.45, -0.32, column, ha="center", va="center", color=colors["muted"], fontweight="bold", fontsize=label_fontsize)
        for row_idx, row in enumerate(rows):
            plate_ax.text(-0.35, row_idx + 0.45, row, ha="center", va="center", color=colors["muted"], fontweight="bold", fontsize=label_fontsize)
            for col_idx, column in enumerate(columns):
                well = f"{row}{column}"
                selected = well in selected_wells
                plate_ax.add_patch(
                    Rectangle(
                        (col_idx, row_idx),
                        0.9,
                        0.9,
                        facecolor=well_color(well),
                        edgecolor=colors["active_border"] if selected else colors["border"],
                        linewidth=3 if selected else 1,
                    )
                )
                pattern = well_pattern(well)
                if pattern:
                    plate_ax.add_patch(
                        Rectangle(
                            (col_idx, row_idx),
                            0.9,
                            0.9,
                            facecolor="none",
                            edgecolor=colors["pattern"],
                            linewidth=0,
                            hatch=pattern,
                            zorder=3,
                        )
                    )
                plate_ax.text(
                    col_idx + 0.45,
                    row_idx + 0.45,
                    well_text(well),
                    ha="center",
                    va="center",
                    color=colors["text"],
                    fontsize=fontsize,
                    linespacing=1.05,
                    zorder=5,
                )
        fig.canvas.draw_idle()

    def draw_marquee(start: tuple[float, float], end: tuple[float, float]) -> None:
        if state["marquee"] is not None:
            state["marquee"].remove()
        x1, y1 = start
        x2, y2 = end
        state["marquee"] = Rectangle(
            (min(x1, x2), min(y1, y2)),
            abs(x2 - x1),
            abs(y2 - y1),
            facecolor=colors["active_border"],
            edgecolor=colors["active_border"],
            alpha=0.18,
            linewidth=1.5,
            zorder=20,
        )
        plate_ax.add_patch(state["marquee"])
        fig.canvas.draw_idle()

    def set_wells(wells: list[str], **values: object) -> None:
        df = data["df"]
        for well in wells:
            idx = row_df_index(well)
            for key, value in values.items():
                df.at[idx, key] = value

    def set_vehicle(wells: list[str]) -> None:
        df = data["df"]
        for well in wells:
            idx = row_df_index(well)
            genotype = df.at[idx, "genotype"] or ""
            df.at[idx, "treatment_type"] = "vehicle"
            df.at[idx, "drug"] = "Vehicle"
            df.at[idx, "gradient_id"] = "vehicle"
            df.at[idx, "gradient_axis"] = ""
            df.at[idx, "concentration_index"] = 0
            df.at[idx, "replicate_group"] = f"{genotype}_vehicle" if genotype else "vehicle"
            df.at[idx, "is_vehicle"] = True

    def set_gradient(wells: list[str]) -> None:
        df = data["df"]
        drug = str(state["selected_drug"])
        axis = str(state["gradient_axis"])
        reverse = bool(state["reverse"])
        ordered = ordered_wells(wells, axis, reverse=reverse)
        lookup = concentration_indices(ordered, axis, reverse=reverse)
        for well in ordered:
            idx = row_df_index(well)
            genotype = df.at[idx, "genotype"] or ""
            concentration_index = lookup[well]
            row, column = split_well(well)
            replicate_key = column if axis == "horizontal" else row
            df.at[idx, "treatment_type"] = "drug_gradient"
            df.at[idx, "drug"] = drug
            df.at[idx, "gradient_id"] = f"{drug}_{axis}"
            df.at[idx, "gradient_axis"] = axis
            df.at[idx, "concentration_index"] = concentration_index
            df.at[idx, "replicate_group"] = f"{genotype}_{drug}_dose_{concentration_index}_{replicate_key}".strip("_")
            df.at[idx, "is_vehicle"] = False

    def clear_wells(wells: list[str]) -> None:
        df = data["df"]
        for well in wells:
            idx = row_df_index(well)
            for column in PLATE_MAP_COLUMNS:
                if column not in {"well", "row", "column"}:
                    df.at[idx, column] = False if column == "is_vehicle" else ""

    def apply_selection(mode: str):
        if not selected_wells:
            set_status("Highlight one or more wells first.")
            return
        wells = ordered_wells(list(selected_wells), str(state["gradient_axis"]), reverse=bool(state["reverse"])) if mode == "drug_gradient" else ordered_wells(list(selected_wells), "horizontal")
        if mode == "genotype_1":
            set_wells(wells, genotype="Genotype 1")
        elif mode == "genotype_2":
            set_wells(wells, genotype="Genotype 2")
        elif mode == "vehicle":
            set_vehicle(wells)
        elif mode == "drug_gradient":
            set_gradient(wells)
        elif mode == "clear":
            clear_wells(wells)
        count = len(selected_wells)
        selected_wells.clear()
        data["df"] = add_replicate_indices(data["df"])
        set_status(f"Applied {mode.replace('_', ' ')} to {count} well(s).")
        redraw_plate()

    def reset_all(event=None) -> None:
        if not state["confirm_reset"]:
            state["confirm_reset"] = True
            set_status("Click Reset All again to confirm clearing every well.")
            return
        data["df"] = blank_map()
        selected_wells.clear()
        state["confirm_reset"] = False
        set_detail("Hover over a well for details.")
        set_status("All wells reset.")
        redraw_plate()

    def sanitized_layout_name() -> str:
        name = textbox_value("layout_name") or "plate_map"
        cleaned = "".join(character if character.isalnum() or character in {"-", "_"} else "_" for character in name).strip("_")
        return cleaned or "plate_map"

    def save_layout(event=None) -> None:
        output_dir.mkdir(parents=True, exist_ok=True)
        path = (output_dir / f"{sanitized_layout_name()}.csv").expanduser().resolve()
        with path.open("w", newline="") as handle:
            writer = csv.writer(handle)
            for row in rows:
                writer.writerow([compact_csv_value(f"{row}{column}") for column in columns])
        set_status(f"Saved layout to {path}")

    def on_press(event) -> None:
        well = well_from_event(event)
        if well is None:
            return
        state["confirm_reset"] = False
        state["dragging"] = True
        state["drag_start"] = (event.xdata, event.ydata)
        state["drag_end"] = (event.xdata, event.ydata)

    def on_motion(event) -> None:
        if event.inaxes is not plate_ax or event.xdata is None or event.ydata is None:
            if not state["dragging"]:
                set_detail(hover_detail(None))
            return
        if not state["dragging"]:
            set_detail(hover_detail(well_from_event(event)))
            return
        state["drag_end"] = (event.xdata, event.ydata)
        draw_marquee(state["drag_start"], state["drag_end"])

    def on_release(event) -> None:
        if not state["dragging"]:
            return
        state["dragging"] = False
        start = state["drag_start"]
        end = (event.xdata, event.ydata) if event.inaxes is plate_ax and event.xdata is not None and event.ydata is not None else state["drag_end"]
        if state["marquee"] is not None:
            state["marquee"].remove()
            state["marquee"] = None
        if start is None or end is None:
            redraw_plate()
            return
        if abs(start[0] - end[0]) < 0.05 and abs(start[1] - end[1]) < 0.05:
            well = well_from_event(event)
            if well:
                if well in selected_wells:
                    selected_wells.remove(well)
                else:
                    selected_wells.add(well)
        else:
            selected_wells.update(wells_in_rectangle(start, end))
        set_status(f"{len(selected_wells)} well(s) highlighted.")
        redraw_plate()

    Button(set_g1_ax, "Set Genotype 1").on_clicked(lambda event: apply_selection("genotype_1"))
    Button(set_g2_ax, "Set Genotype 2").on_clicked(lambda event: apply_selection("genotype_2"))
    Button(vehicle_ax, "Set Vehicle Controls").on_clicked(lambda event: apply_selection("vehicle"))
    Button(gradient_ax, "Set Drug Gradient").on_clicked(lambda event: apply_selection("drug_gradient"))
    Button(clear_ax, "Clear Highlighted").on_clicked(lambda event: apply_selection("clear"))
    Button(reset_ax, "Reset All").on_clicked(reset_all)
    Button(save_ax, "Save Layout").on_clicked(save_layout)

    fig.canvas.mpl_connect("button_press_event", on_press)
    fig.canvas.mpl_connect("motion_notify_event", on_motion)
    fig.canvas.mpl_connect("button_release_event", on_release)
    fig.canvas.mpl_connect("resize_event", lambda event: redraw_plate())
    refresh_drug_toggles()
    redraw_plate()
    plt.show()


def launch_plate_map_gui(
    output_csv: str | Path | None = None,
    use_tk: bool = False,
    use_browser: bool = False,
):
    """Launch the interactive plate-map CSV editor."""
    if use_tk:
        app = PlateMapApp(output_csv=output_csv)
        app.mainloop()
        return None
    if use_browser:
        return launch_plate_map_matplotlib(output_csv=output_csv)
    return launch_plate_map_matplotlib(output_csv=output_csv)


def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Create or validate a CellPyAbility plate map CSV.")
    parser.add_argument("--output", help="Path for the plate map CSV created by the GUI or template.")
    parser.add_argument("--default", action="store_true", help="Write the current default GDA-style plate map without opening the GUI.")
    parser.add_argument("--validate", help="Validate an existing plate map CSV and exit.")
    args = parser.parse_args(argv)

    if args.validate:
        load_plate_map(args.validate)
        print(f"Valid plate map: {args.validate}")
        return

    if args.default:
        if not args.output:
            parser.error("--default requires --output")
        saved = save_plate_map(default_gda_plate_map(), args.output)
        print(f"Saved default plate map: {saved}")
        return

    launch_plate_map_gui(output_csv=args.output)


if __name__ == "__main__":
    main()
